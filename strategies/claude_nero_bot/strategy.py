...
Every turn is instrumented like an F1 data channel.
See telemetry.py for the telemetry system and telemetry.json for saved data.
"""
...
class OpponentProfile:
    ...
    def style(self):
        ...
        agg = sum(self.action_plays.values()) / self.play_count
        if agg > 0.45: return "AGGRESSIVE"
...
EPSILON_MIN    = 0.03
EPSILON_DECAY  = 0.993
_H_PRIORITY    = {"WILD_DRAW_FOUR":0,"DRAW_TWO":1,"SKIP":2,"REVERSE":3,"NUMBER":4,"WILD":5}
...
class ClaudeNeroBot(BaseStrategy):
    ...
    _WEIGHTS_FILE = os.path.join(os.path.dirname(__file__), "nn_weights.json")
    ...
    def _load_weights(self):
        try:
            with open(self._WEIGHTS_FILE) as f: d = json.load(f)
            ...
            print(f"[ClaudeNeroBot] Loaded — {games} games, ε={eps:.3f}, replay={len(replay)}")
            return net, replay, games, eps
        except Exception:
            print("[ClaudeNeroBot] Fresh start")
    ...
    def _reset_game_state(self):
        ...
        # Episode: list of (state_features, shaped_reward)
        self._episode: List[Tuple[List[float], float]] = []
        # Track td_errors from last training batch for telemetry
    ...
    def on_game_start(self):
        super().on_game_start()
        self._reset_game_state()
        self._telem.start_game(self._games_trained, self._epsilon)
    ...
    def on_game_end(self, won: bool, placement: int, points: int):
        ...
        # Monte Carlo discounting — propagate terminal reward back through episode
        discounted = terminal
        returns = []
        for t in reversed(range(len(self._episode))):
            state, shaped = self._episode[t]
            g = discounted + shaped
        ...
        # Telemetry — fault: high epsilon late in training
        if self._games_trained > 100 and self._epsilon > 0.15:
            self._telem.fault("EPSILON_HIGH",
                              f"ε={self._epsilon:.3f} after {self._games_trained} games")

        self._telem.end_game(won, placement, points, terminal, self._last_td_errors)
        self._telem.print_game_summary()
    ...
    def choose_card(
        ...
        **kwargs,
        ...
        # Detect if we were targeted by a draw card
        if last_card and last_card.get("type") in ("DRAW_TWO","WILD_DRAW_FOUR"):
            self._telem.record_targeted(last_card["type"])
        ...
        self._telem.start_turn(self._turn_number)
        color_counts = {c: self.count_color(hand, c) for c in COLORS}
        dominant     = max(color_counts, key=lambda c: color_counts[c]) if hand else "RED"
        self._telem.record_hand(hand, len(playable), dominant, color_counts)
        self._telem.record_opponents(opp_counts, self._pain_color())

        if not playable:
            self._record_draw()
            self._telem.record_draw()
            # Fault: check if we've been stuck on same color
            if self._last_color == current_color:
                self._telem.fault("STUCK_COLOR", current_color)
            self._telem.end_turn()
            return None, None

        # Color change tracking
        if self._last_color and self._last_color != current_color:
            self._telem.record_color_change(self._last_color, current_color)

        # ---- Mode determination ----
        mode = self._determine_mode(len(hand), opp_counts)

        # ---- Fault: LOW_ACTIONS in DEFENSIVE mode ----
        action_count = sum(1 for c in hand
                           if c["type"] in ("SKIP","REVERSE","DRAW_TWO","WILD_DRAW_FOUR"))
        if mode == "DEFENSIVE" and action_count <= 1:
            self._telem.fault("LOW_ACTIONS", f"only {action_count} attack cards in DEFENSIVE")
        ...
        if epsilon_used:
            ...
            chosen_idx, chosen_card, best_nn_score, best_h_score = self._nn_pick(
        ...
        if any(c <= 1 for c in opp_list):
            if chosen_card and chosen_card.get("type") not in \
               ("DRAW_TWO","WILD_DRAW_FOUR","SKIP","REVERSE"):
        ...
        if chosen_card["type"] in ("WILD","WILD_DRAW_FOUR"):
            ...
            # Fault: WILD_WASTED (played wild when we had matching color cards)
            if self.count_color(hand, current_color) > 1:
                self._telem.fault("WILD_WASTED",
                    f"played {chosen_card['type']} with {self.count_color(hand, current_color)} {current_color} cards in hand")

        # ---- NN confidence fault ----
        if best_nn_score > 0 and best_nn_score < 0.35 and self._games_trained > 50:
            self._telem.fault("NN_COLD", f"confidence={best_nn_score:.3f}")

        # ---- Telemetry: record decision ----
        self._telem.record_decision(mode, epsilon_used, best_nn_score, best_h_score, nn_weight)
        self._telem.record_play(chosen_card, wild_color)
        self._telem.end_turn()
    ...
    def _heuristic_score(self, card, hand, next_hand, opp_counts, current_color):
        ...
        counts = list(opp_counts.values()) if opp_counts else \
                 [p.card_count for p in self._profiles.values()]
        danger  = any(c <= 3 for c in counts)
        endgame = hand_size <= 2

        base  = 1.0 - (_H_PRIORITY.get(ctype, 4) / 5.0)
        boost = 0.0
        ...
        if danger:
            ...
            dom = self._dominant_color(hand)
            if ctype == "NUMBER" and ccolor == dom: boost += 0.3
        else:
            if ctype in ("WILD","WILD_DRAW_FOUR"):  boost -= 0.15
            if ctype in ("SKIP","REVERSE","DRAW_TWO"): boost -= 0.1

        dom_next = self._dominant_color(next_hand) if next_hand else "RED"
        if ccolor == dom_next: boost += 0.1
    ...
    def _encode_state(self, hand, top_card, current_color, opp_counts):
        ...
        ocl    = list(opp_counts.values()) if opp_counts else \
                 [p.card_count for p in self._profiles.values()]
        min_o  = min(ocl) if ocl else 7
        max_o  = max(ocl) if ocl else 7
        ...
        return [
            ...
            1.0 if dominant==current_color else 0.0,
